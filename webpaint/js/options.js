...
!(function () {
  var e = localStorage.getItem("config"),
    ...
    o = document.getElementById("save"),
    c = null;
  try {
    c = JSON.parse(e);
  } catch (e) {}
  function t(e) {
    for (
      var t, a, s = this.value.toUpperCase().replace(/([^0-9A-Z])/g, ""), l = 0;
      l < i.length;
      l++
    ) {
      var n = i[l];
    ...
    (this.value = s),
      (c.hotkeys && c.hotkeys[this.id] === this.value) ||
        ((a = "hide"),
        ((t = o).className = t.className.replace(
          new RegExp("\\b" + a + "\\b", "g"),
          ""
        )));
  }
